...
type gatewayAPITranslator struct {
	...
	gl4cTranslator translation.GL4CTranslator
	cfg            translation.Config
}

func NewTranslator(cecTranslator translation.CECTranslator, gl4cTranslator translation.GL4CTranslator, cfg translation.Config) translation.Translator {
	return &gatewayAPITranslator{
		cecTranslator:  cecTranslator,
		gl4cTranslator: gl4cTranslator,
		cfg:            cfg,
	}
}

func (t *gatewayAPITranslator) Translate(m *model.Model) (*ciliumv2.CiliumEnvoyConfig, *ciliumv2alpha1.CiliumGatewayL4Config, *corev1.Service, error) {
	...
	var gl4c *ciliumv2alpha1.CiliumGatewayL4Config
	if m.IsL4ListenerConfigured() {
		gl4c, err = t.gl4cTranslator.Translate(source.Namespace, shortener.ShortenK8sResourceName(generatedName), m)
		if err != nil {
			return nil, nil, nil, err
		}
	}
	...
	if gl4c != nil {
		if err = decorateGL4C(gl4c, owner, allLabels, allAnnotations); err != nil {
			return nil, nil, nil, err
		}
	}
...
func decorateGL4C(gl4c *ciliumv2alpha1.CiliumGatewayL4Config, resource *model.FullyQualifiedResource, labels, annotations map[string]string) error {
	if gl4c == nil || resource == nil {
		return fmt.Errorf("GL4C or resource can't be nil")
	}

	// Set the owner reference to the CEC object.
	gl4c.OwnerReferences = []metav1.OwnerReference{
		{
			APIVersion: resource.Group + "/" + resource.Version,
			Kind:       resource.Kind,
			Name:       resource.Name,
	...
	if gl4c.Labels == nil {
		gl4c.Labels = make(map[string]string)
	}
	gl4c.Labels = mergeMap(gl4c.Labels, labels)
	gl4c.Labels[gatewayNameLabel] = shortener.ShortenK8sResourceName(resource.Name)
	gl4c.Annotations = mergeMap(gl4c.Annotations, annotations)
