...
func (c *gatewayL4Controller) processGatewayL4(wtxn writer.WriteTxn, watches *statedb.WatchSet) {
	...
	for cfg := range cfgs {
		...
		backendMap := make(map[loadbalancer.L3n4Addr]*loadbalancer.Backend)
		...
		currentBackends := make([]loadbalancer.Backend, 0, len(backendMap))
...
func (c *gatewayL4Controller) collectBackends(
	...
	backendMap map[loadbalancer.L3n4Addr]*loadbalancer.Backend,
) {
	...
	for _, be := range listener.Backends {
		...
		bes, beWatch := loadbalancer.ListBackendsByServiceName(txn, c.p.Writer.Backends(), backendService)
		watches.Add(beWatch)

		for backend := range bes {
			if backend.Address.Protocol() != proto {
				continue
			}

			// Check if backend already exists (from another listener)
			if existing, exists := backendMap[backend.Address]; exists {
				// Merge PortNames from multiple listeners
				existing.PortNames = append(existing.PortNames, gatewayPortName)
			} else {
				// Add new backend
				backendMap[backend.Address] = &loadbalancer.Backend{
...
func (c *gatewayL4Controller) upsertGWBackends(
	...
	backendParams []loadbalancer.Backend,
) error {
	// Get current backends for this Gateway
	currentBackends := make(map[loadbalancer.L3n4Addr]*loadbalancer.Backend)
	...
	backends := func(yield func(loadbalancer.Backend) bool) {
		for _, params := range backendParams {
			if !yield(params) {
				return
			}
